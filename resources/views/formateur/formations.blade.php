...
<div class="row mt-2 mb-1">
    @if (isset($autresformations) && $autresformations->count() > 0)
        <div class="row">
            <div class="col-lg-9 col-md-8 col-sm-6 mb-4">
                <h4 class="mb-2">Formations disponibles : [ <span id="formations-count"> {{ $autresformations->count() }} </span> ]</h4>
            </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                function fetchFormation(query) {
                    ...
                    fetch(`{{ route('fmt-search-formations') }}?recherche=${encodeURIComponent(query)}`)
                        .then(res => res.json())
                        .then(data => {
